 ...
 * 
 * 创建人：litaijun
 * 创建时间：2015-3-28 下午03:12:24
 * 修改人：
 * 修改时间：2015-3-28 下午03:12:24
 * 修改备注：
 * 
 * @version 1.0.0
 */
...
//		assertThat(Years.yearsBetween(birthDate, new DateTime()).getYears()).isEqualTo(10);
		...
		System.out.println(birthDate);
		birthDate = now.plusYears(1).plusMonths(2).plusDays(3).plusHours(4).plusMinutes(5).plusSeconds(6);
		System.out.println(birthDate);
...
//		DateTime startOfMonth = dt.dayOfMonth().withMinimumValue().withDayOfMonth(12);
		DateTime startOfMonth = dt.monthOfYear().withMinimumValue().withDayOfWeek(2);
